...
import htap_functions.htap_functions as ht_fn
import HTAP1_functions.H1_funcs as ht1_fn
import HTAP2_functions.H2_funcs as ht2_fn
...
# EMISSION SCENARIO
EMIS_SCN  = 'Emis_scn_CMIP5_RCP26'   # used for naming output files
EMISSIONS = ['NOx','VOC','CO']       # Emissions precursors to be perturbed
SPEC      = 'o3'                     # output species to be scaled
YEARS_SCN = ['2030']                 # Specify years considered within specific scenario relative to 2010
...
EMIS_SCN_FNAME_H1 = 'H1_emis_CMIP5_RCP26_all_sects_TOTAL_2000_BASE_2030.dat'                           # Emission changes for HTAP1 source regions
EMIS_SCN_FNAME_H2 = 'H2_emis_CMIP5_RCP26_all_sects_TOTAL_2000_BASE_2030.dat'                           # Emission changes for HTAP2 Source Regions
...
# OUTPUT DIRECTORIES
OUT_TXT_FILE_PATH   = '/home/h06/sturnock/Python/HTAP/para_o3_model/output_txt_files/CMIP5_ALL_YRS/RCP26/'  # Path for regional mean output
OUT_FILE_PATH       = '/data/users/sturnock/HTAP_files/para_o3_model/CMIP5_ALL_YRS/'                        # Path for netcdf output of scaled O3 response
PLOT_DIR            = '/home/h06/sturnock/Python/HTAP/para_o3_model/output_images/CMIP5_ALL_YRS/RCP26/'     # Path for regional change plots
...
IOUT_TXT_MMM_H1  = 0    # output text file for regional HTAP1 multi-model mean response + std devs
IOUT_TXT_MMM_H2  = 0    # output text file for regional HTAP2 multi-model mean response + std devs to all regional emis scenarios   
IOUT_NCF_INDIVID_H1 = 0 # set to output netcdf files of INDIVIDUAL HTAP1 MODEL responses
IOUT_TXT_INDIVID_H1 = 0 # output text file for regional responses from each INDIVIDUAL HTAP1 model 
IOUT_TXT_INDIVID_H2 = 0 # output text file for regional responses from each INDIVIDUAL HTAP2 model
...
if __name__ == '__main__':
    ...
    for (imod,mod_file) in enumerate(MODEL_FNAMES_H1):
        ...
        for (iyr,year) in enumerate(YEARS_SCN):
            ...
            for (irun,run) in enumerate(H1_RUNID): #loop through each scenario of ozone changes (base - 20% reduction)
                ...
                o3_mod_burd, mod_burd_dob, mod_rf_glob, mod_burd_htap2_regs, mod_rf_htap2_regs = ht_fn.calc_burd_ACCMIP(o3_3d_data_sing_reg,o3_3d_data_SR1,dp_3d,grid_areas_mon_3d,accmip_2d_rf,NREGS_H2_OUT,HTAP2_RECP_REGS)
                
                o3_burd_all_mod_H1[imod,iyr,ireg]       = o3_mod_burd
                rf_glob_all_mod_H1[imod,iyr,ireg]       = mod_rf_glob
                reg_o3_burd_all_mod_H1[imod,iyr,ireg,:] = mod_burd_htap2_regs[:]
                reg_o3_rf_all_mod_H1[imod,iyr,ireg,:]   = mod_rf_htap2_regs[:]
    ...
    for (iyr,yr) in enumerate(YEARS_SCN):
        ...
        o3_burd_all_mod_mmm_comb_H1_H2_ch4[iyr] = np.sum(o3_burd_all_mod_H2_mmm[iyr,1:]) + np.sum(o3_burd_all_mod_H1_mmm[iyr,:]) + global_mean_burd_resp_H1_H2_ch4_comb_mmm[iyr]
        rf_glob_all_mod_mmm_comb_H1_H2_ch4[iyr] = np.sum(rf_glob_all_mod_H2_mmm[iyr,1:]) + np.sum(rf_glob_all_mod_H1_mmm[iyr,:]) + global_mean_rf_resp_H1_H2_ch4_comb_mmm[iyr]
        ...
        for (ireg_h2,reg_h2) in enumerate(sorted(HTAP2_RECP_REGS)):
            ...
            o3_burd_reg_all_mod_mmm_comb_H1_H2_ch4[iyr,ireg_h2] = np.sum(reg_o3_burd_all_mod_H2_mmm[iyr,1:,ireg_h2]) + np.sum(reg_o3_burd_all_mod_H1_mmm[iyr,:,ireg_h2]) + reg_mean_burd_resp_H1_H2_ch4_comb_mmm[iyr,ireg_h2]
            rf_reg_all_mod_mmm_comb_H1_H2_ch4[iyr,ireg_h2]      = np.sum(reg_o3_rf_all_mod_H2_mmm[iyr,1:,ireg_h2]) + np.sum(reg_o3_rf_all_mod_H1_mmm[iyr,:,ireg_h2]) + reg_mean_rf_resp_H1_H2_ch4_comb_mmm[iyr,ireg_h2]
